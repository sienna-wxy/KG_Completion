...
class TransH(KGEModel):
    ...
    def init_emb(self):
        ...
        self.norm_vector = nn.Embedding(self.args.nrelation, self.args.emb_dim)
    ...
    def forward(self, triples, mode="single"):
        if mode == "single":
            ...
            head_emb = torch.index_select(self.entity_embedding.weight.data, dim=0, index=head_part.view(-1)).view(
                batch_size, negative_sample_size, -1)  # [bs, num_neg, dim]
            ...
            norm_vector = self.norm_vector(tail_part[:, 1]).unsqueeze(dim=1)  # shape:[bs, 1, dim]
            ...
            tail_emb = torch.index_select(self.entity_embedding.weight.data, dim=0, index=tail_part.view(-1)).view(
                batch_size, negative_sample_size, -1)  # [bs, num_neg, dim]
            norm_vector = self.norm_vector(head_part[:, 1]).unsqueeze(dim=1)  # shape:[bs, 1, dim]
